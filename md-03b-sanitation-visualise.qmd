...
3.  How many variables are in the data set?
    ...
    There are 8 variables (Represented by the number of columns)
...
```{r}
sanitation_uganda_sm_2000_2020 <- sanitation |> 
  filter(iso3 == "UGA",
         year == 2000 | year == 2020,
...
```{r}
sanitation_uganda_sm_2000_2020 |> 
  count(year)
```
...
```{r}
  ggplot(data = sanitation_uganda_sm_2000_2020,
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    The bars are supposed to be arranged starting from the poor sanitation facilities to the basic ones ( which are partially improved). This implies that the "no sanitation facilities" was supoosed to be at the bottom of the bar

2.  What would you want to change?

    I would want to change the arrangement of the data so that the "no sanitation facilities" part comes at the bottom of the bars. Also, i would want to participate in changing of all those data sizes so that we increase the size of the "basic sanitation services" and probably the "safely managed sanitation services"

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    "Safely managed sanitation services" pose no health risk thus the most critical areas that require working on are the ones we remained with.
...
2.  What do you observe when you look at the code and plot?
    ...
    The grids represent different plots for each combination of "varname_short" and "residence"

    The code provided creates a histogram using ggplot. It filters the data set "sanitation" to include only the data for the year 2020 and then creates a new object "sanitation_2020". A histogram is created based on the filtered data.

    A faceted grid is then created from the histogram for each combination of "varname_short" and "residence". The scales on the y-axes of the created plots are allowed to vary
